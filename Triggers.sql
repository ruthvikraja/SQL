...
BEFORE INSERT ON salaries # Before INSERTING a particular value to a column this TRIGGER is "triggered"
FOR EACH ROW 
BEGIN 
	IF NEW.salary < 0 THEN  # Here NEW.salary refers to the newly created data in the salary column
		SET NEW.salary = 0; # Used to SET value to a particular variable
	END IF; 
END $$
...
BEGIN 
	IF NEW.salary < 0 THEN 
		SET NEW.salary = OLD.salary; # NEW and OLD are inbuilt keywords
	END IF; 
END $$
